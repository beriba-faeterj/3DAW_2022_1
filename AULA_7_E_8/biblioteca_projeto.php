...
$arrayValidacao = array(
    array("/^([1-9][0-9]*|0)?$/", "O ID do produto deve ser um número inteiro não nulo, sem começar por 0."),
    array("/^([A-z][A-z ]*|0)?$/", "O nome do produto deve possuir apenas letras, sem acento."),
    array("/^([1-9][0-9]*|0)?$/", "O código de barras do produto deve ser um número inteiro."),
    array("", ""), // Descrição pode ter letras, números, etc.
    array("~(http.*\.)(jpe?g|png|gif|[tg]iff?|svg)~i", "A URL inserida não é válida."),
    array("/^([1-9][0-9]*|0)(\,[0-9]{2})?$/", "O valor do produto deve seguir o formato ...X,XX."),
    array("/^([1-9][0-9]*|0)?$/", "A quantidade do produto deve ser um número inteiro."),
    array("/^([1-9][0-9]*|0)(\,[0-9]{2})?$/", "O peso do produto deve seguir o formato ...X,XX (kg).")
);
